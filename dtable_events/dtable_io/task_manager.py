...
class TaskManager(object):
    ...
    def add_convert_view_to_execl_task(self, dtable_uuid, table_id, view_id, username, id_in_org, permission, name, archive_view_export_row_limit):
        from dtable_events.dtable_io import convert_view_to_execl

        task_id = str(int(time.time()*1000))
        task = (convert_view_to_execl, (dtable_uuid, table_id, view_id, username, id_in_org, permission, name, archive_view_export_row_limit))
        self.tasks_queue.put(task_id)
        self.tasks_map[task_id] = task
