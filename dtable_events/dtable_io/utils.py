...
def get_emails(send_date, email_server, email_user, email_password, port=None, mode='ON', timeout=None):
    """
    return: email list, [email1, email2...], email is without thread id
    """
    from dtable_events.utils.imap_mail import ImapMail
    from dtable_events.dtable_io import dtable_io_logger

    imap = ImapMail(email_server, email_user, email_password, port=port, ssl_context=ssl.SSLContext(ssl.PROTOCOL_TLSv1_2), timeout=timeout)
    imap.client()
    dtable_io_logger.debug('imap: %s client successfully!', email_server)
    imap.login()
    dtable_io_logger.debug('email_server: %s email_user: %s, password: %s login imap client successfully!', email_server, email_user, email_password)
    ...
    for email in email_list:
        ...
        for file in file_list:
            ...
            try:
                ...
                dtable_io_logger.error('upload email: %s attachment: %s error: %s', email.get('Message ID'), file_name, e)
...
def get_seatable_api(username, dtableUuid):
    from seatable_api import SeaTableAPI

    DTABLE_WEB_URL = task_manager.conf['dtable_web_service_url']
    ...
    seatable = SeaTableAPI(api_token, DTABLE_WEB_URL)
    seatable.auth()
