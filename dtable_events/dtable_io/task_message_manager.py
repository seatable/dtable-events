...
class TaskMessageManager(object):
    ...
    def add_sms_sending_task(self,dtable_uuid, username, org_id, phone, template_name, msg_dict):
        from dtable_events.dtable_io import send_sms_msg
        task_id = str(int(time.time() * 1000))
        task = (send_sms_msg, (dtable_uuid, username, org_id, phone, template_name, msg_dict, self.config))
        self.tasks_queue.put(task_id)
        self.tasks_map[task_id] = task
        return task_id
