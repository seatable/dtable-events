...
class AutoRulesStatsUpdater:
    ...
    def update_user(self, db_session, username):
        ...
        if not has_sent_warning and usage >= limit * 0.9:
            self.dtable_web_api.internal_add_notification([username], 'autorule_limit_reached_warning', {'limit': limit, 'usage': usage})
    ...
    def update_org(self, db_session, org_id):
        ...
        if not has_sent_warning and usage >= limit * 0.9:
            ...
            self.dtable_web_api.internal_add_notification(admins, 'autorule_limit_reached_warning', {'limit': limit, 'usage': usage})
