...
monitor_logger = setup_logger('browser-monitor.log', propagate=False)
...
class ProcessMonitor(Thread):
    ...
    def run(self):
        if not self.can_monitor():
            monitor_logger.debug("No monitoring!")
            return
        monitor_logger.debug("Starting monitoring...")
        self.running = True
        while self.running:
            for pid in list(self.pid_infos.keys()):
                ...
                try:
                    ...
                    monitor_logger.debug(f"pid: {self.pid_infos[pid]} not exists, removed!")
                    self.pid_infos.pop(pid, None)  # remove when pid not exists longer
                    continue

                total_memory_mb = total_memory / (1024 * 1024)  # convert to MB
                monitor_logger.debug(f"pid: {self.pid_infos.get(pid)} Total CPU: {total_cpu:.2f}%, Total Memory: {total_memory_mb:.2f} MB")
